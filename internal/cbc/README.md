# CBC Chain Integration for CBCScan Explorer

This package provides CBC Chain specific initialization and runtime handling for the CBCScan blockchain explorer.
...
5. Integrates seamlessly with CBCScan's existing architecture
...
The CBC initialization is integrated into CBCScan's service layer:
...
When the CBCScan service starts, it checks if the network is CBC Chain:
...
The metadata is registered with CBCScan's codec system:
...
The CBC initialization uses the existing CBCScan configuration:
...
- Falls back to standard CBCScan behavior if CBC init fails
...
- Integrated with CBCScan architecture
...
1. CBCScan service starts (API, Observer, Worker)
...
Same as CBCScan Explorer (Apache 2.0)
